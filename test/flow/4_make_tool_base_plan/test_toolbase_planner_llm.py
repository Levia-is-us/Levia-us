...
def check_case_fail(output, expect):
    ...
    # Check tool names
    tools = ['WebSearchTool', 'WebsiteScanTool', 'SaveMarkdownToGitbook', 'ListAbilitiesTool']
    if status == "success":
        plans = output.get("plan")
        for plan in plans:
...
def save_results_to_json(results):
    ...
    save_path = Path(__file__).parent / f"toolbase_planner_test_{timestamp}.json"
