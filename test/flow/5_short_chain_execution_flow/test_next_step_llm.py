...
from next_step_llm_schema import next_step_proceed_schema, cannot_proceed_schema
...
def run_single_test(model, test_case, idx):
    ...
    can_proceed = test_case["expect"]["can_proceed"]
    if not can_proceed:
        tool_config = {
            "method": "add",
            "inputs": [
                {
                    ...
                    "type": "int",
                    "required": True,
                    "description": "Addition of a number",
                }
            ],
            "output": {
...
def linear_execute(cases: list):
    ...
    for test_case in cases:
        expect = test_case["expect"]["can_proceed"]
        for model in models:
...
def check_case_fail(output, expect):
    ...
    # Status field check
    can_proceed = output.get("can_proceed", None)
    if can_proceed is None:
        return "Failed: missing required field: can_proceed"

    if can_proceed != expect:
        return f"Failed: output can_proceed status {can_proceed} does not match expected status {expect}"

    # Select schema based on status type
    schema_map = {
        True: next_step_proceed_schema,
        False: cannot_proceed_schema,
    }

    schema = schema_map.get(can_proceed)
    if not schema:
        return f"Failed: unknown can_proceed status: {can_proceed}"
...
def save_results_to_json(results):
    ...
    save_path = Path(__file__).parent / f"next_step_test_{timestamp}.json"
