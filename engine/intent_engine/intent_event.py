...
def event_chat(user_id, input_message, session_id: str = ""):
    ...
    try:
        ...
        reply = backup_reply(short_term_memory.get_context(user_id + session_id), user_id, chid)
        output_stream(log=f"{reply}", user_id=user_id, type="think", ch_id=chid, title="Final reply")
