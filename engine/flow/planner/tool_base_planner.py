...
def tool_base_planner(intent: str, tool_list: list, user_id: str):
    tool_base_planner_prompt = get_tool_base_planner_prompt(intent, tool_list)
    prompt = [
        {"role": "user", "content": tool_base_planner_prompt},
    ]
    plan = chat_completion(prompt, model=QUALITY_MODEL_NAME, config={"temperature": 0.7}, user_id=user_id)
