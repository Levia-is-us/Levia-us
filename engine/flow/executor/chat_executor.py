...
def process_existing_memories(
    ...
    handle_new_tool_execution(plan, messages_history, user_id)
...
def handle_new_tool_execution(
    execution_records_str, summary, plan, messages_history: list, user_id: str
) -> list:
    """
    ...
    for step_index, step in enumerate(plan):
        ...
        if not _process_plan_step(
            step, plan, messages_history, step_index, user_id, found_tools
        ):
...
def execute_plan_steps(
    plan_steps, messages_history, execution_records_str, user_id: str
):
    tool_results = []
...
def _process_plan_step(
    step, plan, messages_history, step_index, user_id: str, found_tools
):
    """
...
def _execute_plan_step_tool(
    step, messages_history, execution_records, plan_steps, user_id: str, step_index: int
):
    """
    ...
    while True:
        execution_result = _try_execute_tool(
            tool_config, messages_history, plan_steps, step
        )
...
def _execute_tool_with_args(tool_config, reply_json):
    ...
    result = execute_tool(
        tool_caller_client, f"{tool_config['method']}_tool", tool_config["method"], args
    )
