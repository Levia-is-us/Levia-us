...
def episodic_check(user_intent, context, plan, user_id, ch_id):
    QUALITY_MODEL_NAME = os.getenv("QUALITY_MODEL_NAME")
    CHAT_MODEL_NAME = os.getenv("CHAT_MODEL_NAME")
    prompt = episodic_check_prompt(user_intent, context, plan)
    result = chat_completion(prompt, QUALITY_MODEL_NAME, config={"temperature": 0, "max_tokens": 4000}, user_id=user_id, ch_id=ch_id)
    start_tag = "<think>"
    end_tag = "</think>"
